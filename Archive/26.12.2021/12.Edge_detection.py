...
def image_edge_detection(image_as_numpyarray, RGB_tolerance):
    ...
    for y in range(img_height):
        for x in range(img_width):
            ...
            if different_colour:
                predominant_colour = block_predominant_colour(numpydata, current_pixel, block_size)
                stand_out_pixel = weakest_background_match(previous_pixel, current_pixel, predominant_colour)
                if stand_out_pixel == current_pixel:
                    edges_lat.append([x, y])
                elif stand_out_pixel == previous_pixel:
                    edges_lat.append([x - 1, y])
    print('checkmark1')
